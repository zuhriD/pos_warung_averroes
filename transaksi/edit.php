        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6 bg-surface-container-lowest">
            ...
            <div class="w-full max-w-5xl">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between mb-lg gap-4">
                    <div>
                        <h1 class="font-headline-lg text-on-surface">Edit Transaksi</h1>
                        <p class="text-body-md text-on-surface-variant">Edit transaksi to your store's transaksis.</p>
                    </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-md">
                                <div class="md:col-span-2 flex flex-col gap-xs">
                                    <form action="action.php?aksi=update" method="post">
                                        ...
                                        <label class="font-label-md text-on-surface-variant">User / Kasir</label>
                                        <select name="id_user" class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none">
                                            <?php
                                            $users = showDataUser($conn);
                                            while ($u = $users->fetch_assoc()) {
                                                $selected = ($u['id'] == $data['id_user']) ? 'selected' : '';
                                                echo "<option value='{$u['id']}' $selected>{$u['nama']}</option>";
                                            }
                                            ?>
                                        </select>
                                        
                                        <label class="font-label-md text-on-surface-variant mt-3">Total Pembelian</label>
                                        <input class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" placeholder="e.g. 150000" type="number" name="total_pembelian" value="<?= $data['total_pembelian'] ?>" />
                                        
                                        <label class="font-label-md text-on-surface-variant mt-3">Tanggal Transaksi</label>
                                        <input class="w-full p-md border border-outline-variant rounded-lg font-body-md focus:ring-2 focus:ring-primary-container focus:outline-none" type="date" name="tgl_transaksi" value="<?= $data['tgl_transaksi'] ?>" />
